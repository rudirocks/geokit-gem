  ...
  module Geocoders
    ...
    class GoogleGeocoder < Geocoder
      ...
      def self.do_geocode(address, options = {})
        ...
        res = self.call_geocoder_service("https://maps.googleapis.com/maps/api/geocode/xml?address=#{Geokit::Inflector::url_escape(address_str)}&oe=utf-8&sensor=false")
        return GeoLoc.new if !res.is_a?(Net::HTTPSuccess)
        xml = res.body
        logger.debug "Google geocoding. Address: #{address}."
        logger.debug "Google geocoding. Xml: #{xml}."
